...
static brightness_set_req brightness_set_req_instance;
static brightness_set_res brightness_set_res_instance;
...
static brightness_error_t read_int_file(const char *path, int *out)
{
  ...
  if (!fp) {
    if (errno == EACCES || errno == EPERM) return BRIGHTNESS_ERR_PERMISSION;
    if (errno == ENOENT) return BRIGHTNESS_ERR_NOT_SUPPORTED;
...
{
  if (percent < 0 || percent > 100) return BRIGHTNESS_ERR_INVALID_VALUE;
  int max = 0;
  brightness_error_t err = read_int_file(BRIGHTNESS_SYSFS_MAX_BRIGHTNESS_PATH, &max);
  if (err != BRIGHTNESS_ERR_OK) return err;
  if (max <= 0) return BRIGHTNESS_ERR_DEVICE_ERROR;
