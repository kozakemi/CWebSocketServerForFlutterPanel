...
void brightness_set(struct lws *wsi, size_t index, cJSON *root)
{
    ...
    // 构建响应
    brightness_set_res_instance.type = brightness_dispatch_get_by_index(index)->response;
    ...
    {
        cJSON_AddNumberToObject(res_data, "brightness",
                                brightness_set_req_instance.data.brightness);
    }
